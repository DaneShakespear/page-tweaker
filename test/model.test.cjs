...
test('normalizes visual control values into CSS', () => { assert.equal(cssValue('font-size', 24), '24px'); assert.equal(cssValue('font-weight', 700), '700'); assert.equal(cssValue('line-height', 1.25), '1.25'); assert.equal(cssValue('color', '#ffffff'), '#ffffff'); });
