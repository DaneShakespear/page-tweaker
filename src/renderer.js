...
async function captureScreenshot() {
  const timeout = new Promise((_, reject) => setTimeout(() => reject(new Error('Screenshot capture timed out.')), 1500));
  const image = await Promise.race([page.capturePage(), timeout]);
  const base = `data:image/png;base64,${image.toPNG().toString('base64')}`;
  const imageElement = new Image();
  await new Promise((resolve, reject) => { imageElement.onload = resolve; imageElement.onerror = reject; imageElement.src = base; });
  const composite = document.createElement('canvas');
  composite.width = imageElement.width; composite.height = imageElement.height;
  const compositeContext = composite.getContext('2d');
  compositeContext.drawImage(imageElement, 0, 0);
  compositeContext.drawImage(canvas, 0, 0, composite.width, composite.height);
