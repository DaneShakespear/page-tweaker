...
if (!singleInstance) app.quit();
  ...
  app.whenReady().then(() => {
    ...
    ipcMain.handle('export-bundle', async (_event, bundle) => { const destination = path.join(app.getPath('downloads'), 'Page Tweaker Handoffs'); const stamp = new Date().toISOString().replace(/[:.]/g, '-'); const name = `page-tweaker-handoff-${stamp}`; const folder = path.join(destination, name); const archive = `${folder}.zip`; await fs.mkdir(folder, { recursive: true }); await fs.writeFile(path.join(folder, 'handoff.json'), JSON.stringify(bundle.handoff, null, 2)); await fs.writeFile(path.join(folder, 'START-HERE.md'), bundle.prompt); if (bundle.markupOverlay) await fs.writeFile(path.join(folder, 'markup-overlay.png'), Buffer.from(bundle.markupOverlay.split(',')[1], 'base64')); await execFileAsync('/usr/bin/ditto', ['-c', '-k', '--sequesterRsrc', '--keepParent', folder, archive]); await fs.rm(folder, { recursive: true, force: true }); return { path: archive, name: path.basename(archive) }; });
