...
function sourceFromArgs(args) { for (const arg of args) { if (/^https?:\/\//i.test(arg) || /^file:\/\//i.test(arg)) return arg; if (/^page-tweaker:\/\//i.test(arg)) return sourceFromProtocol(arg); if (/\.(html?|webloc|png|jpe?g|gif|webp|bmp|svg)$/i.test(arg)) return arg; } return null; }
...
function createWindow() { window = new BrowserWindow({ width: 1440, height: 940, minWidth: 980, minHeight: 680, resizable: true, maximizable: true, fullscreenable: true, backgroundColor: '#111318', titleBarStyle: 'hiddenInset', webPreferences: { preload: path.join(__dirname, 'shell-preload.cjs'), contextIsolation: true, webviewTag: true } }); window.loadFile(path.join(__dirname, 'index.html'), { query: pendingSource ? { source: pendingSource } : {} }); }
...
if (!singleInstance) app.quit();
  ...
  app.whenReady().then(() => {
    ...
    ipcMain.handle('choose-source', async () => { const result = await dialog.showOpenDialog(window, { properties: ['openFile'], filters: [{ name: 'Web pages and images', extensions: ['html', 'htm', 'webloc', 'png', 'jpg', 'jpeg', 'gif', 'webp', 'bmp', 'svg'] }] }); return result.canceled ? null : resolveSource(result.filePaths[0]); });
    ipcMain.handle('clipboard-image', async () => { const image = clipboard.readImage(); if (image.isEmpty()) return null; const target = path.join(app.getPath('temp'), `page-tweaker-clipboard-${Date.now()}.png`); await fs.writeFile(target, image.toPNG()); return `file://${encodeURI(target)}`; });
    ipcMain.handle('app-version', () => app.getVersion());
    ipcMain.handle('export-bundle', async (_event, bundle) => { const destination = path.join(app.getPath('downloads'), 'AI PagePolish by PageTweaker Handoffs'); const stamp = new Date().toISOString().replace(/[:.]/g, '-'); const name = `page-tweaker-handoff-${stamp}`; const folder = path.join(destination, name); const archive = `${folder}.zip`; await fs.mkdir(folder, { recursive: true }); await fs.writeFile(path.join(folder, 'handoff.json'), JSON.stringify(bundle.handoff, null, 2)); await fs.writeFile(path.join(folder, 'START-HERE.md'), bundle.prompt); for (const [breakpoint, image] of Object.entries(bundle.annotatedImages || {})) await fs.writeFile(path.join(folder, `${breakpoint}-annotated.png`), Buffer.from(image.split(',')[1], 'base64')); await execFileAsync('/usr/bin/ditto', ['-c', '-k', '--sequesterRsrc', '--keepParent', folder, archive]); await fs.rm(folder, { recursive: true, force: true }); return { path: archive, name: path.basename(archive) }; });
